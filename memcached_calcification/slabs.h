...
/* Calcification: struct for hottest slab and coldest page tracking */
typedef struct hotness_tracker{
	unsigned int slab; /* slab class */
    unsigned int page; /* actual slab - 1 MB */
    unsigned long nhits; /* number of hits for the page */
    unsigned long long time; /* average access time of all the items in that page */
    unsigned int miss_rate; /* misses for the entire slab class divided by total no. of the pages in that slab */
} hotness_tracker;

void init_hotness_tracker(hotness_tracker *tmp);

hotness_tracker coldest_page_info, hottest_slab;
