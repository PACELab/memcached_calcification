...
/* Calcification: page_stats_t */
typedef struct {
	unsigned int nhits;
	rel_time_t access_time;
} page_stats_t;
...
typedef struct {
	...
	/* Calcification */
	page_stats_t *slab_stats; /* array of page_stats*/
	unsigned int prev_misses; /* previous iteration of page_hotness check miss count */
...
/* Calcification */
void init_hotness_tracker(hotness_tracker *tmp) {
	tmp->miss_rate = 0;
	tmp->nhits = 0;
	tmp->time = 0;
	tmp->page = 0;
...
void slabs_init(const size_t limit, const double factor, const bool prealloc,
	...
	/* Calcification */
	init_hotness_tracker(&coldest_page_info);
	init_hotness_tracker(&hottest_slab);
}
...
static int grow_slab_list(const unsigned int id) {
	...
	if (p->slabs == p->list_size) {
		...
		/* Calcification */
		void *new_page_stats = realloc(p->slab_stats,
				new_size * sizeof(page_stats_t));
		if (new_page_stats == 0)
			return 0;
		p->slab_stats = new_page_stats;
	}
	return 1;
}
...
static int do_slabs_newslab(const unsigned int id) {
	...
	if ((grow_slab_list(id) == 0)
			|| (((ptr = get_page_from_global_pool()) == NULL) && ((ptr =
					memory_allocate((size_t) len)) == 0))) {
	...
	/* Calcification */
	p->slab_stats[p->slabs].nhits = 0;
	p->slab_stats[p->slabs].access_time = 0;
...
bool get_stats(const char *stat_type, int nkey, ADD_STAT add_stats, void *c) {
	...
	if (add_stats != NULL) {
		if (!stat_type) {
			...
			STATS_LOCK();
			APPEND_STAT("bytes", "%llu",
					(unsigned long long )stats_state.curr_bytes);
			APPEND_STAT("curr_items", "%llu",
					(unsigned long long )stats_state.curr_items);
			APPEND_STAT("total_items", "%llu",
					(unsigned long long )stats.total_items);
			STATS_UNLOCK();
			if (settings.slab_automove > 0) {
				pthread_mutex_lock(&slabs_lock);
				APPEND_STAT("slab_global_page_pool", "%u",
						slabclass[SLAB_GLOBAL_PAGE_POOL].slabs);
				pthread_mutex_unlock(&slabs_lock);
			}
...
static void do_slabs_stats(ADD_STAT add_stats, void *c) {
	...
	for (i = POWER_SMALLEST; i <= power_largest; i++) {
		...
		if (p->slabs != 0) {
			...
			total++;
			/* Calcification */
			APPEND_NUM_STAT(i, "misses:", "%llu",
								(unsigned long long )thread_stats.slab_stats[i].set_cmds - p->prev_misses);
			char page[30];
			int page_num;
			for (page_num = 0; page_num < p->slabs; page_num++) {
				sprintf(page, "page-%d: nHits:%u ", page_num, p->slab_stats[page_num].nhits);
				APPEND_NUM_STAT(i, page, "time:%u", p->slab_stats[page_num].access_time);
			}
		}
	}

	APPEND_STAT("hottest slab", "%u", (unsigned int)hottest_slab.slab);
	APPEND_STAT("coldest page", "%u", (unsigned int)coldest_page_info.page);
	APPEND_STAT("coldest page belong to the slab class", "%u", (unsigned int)coldest_page_info.slab);
...
static void memory_release() {
	...
	while (mem_malloced > mem_limit && (p = get_page_from_global_pool()) != NULL) {
		free(p);
...
void slabs_stats(ADD_STAT add_stats, void *c) {
	/* Calcification */
	// TODO: Call  to update all pages statistics.
	// page_hotness(10);
...
static int slab_rebalance_start(void) {
	if(settings.verbose >= 1) {
		printf("In slab_rebalance_start\n");
	}
	slabclass_t *s_cls;
	int no_go = 0;
	...
	if (settings.slab_automove == 3) {
		// cp = coldest_page_info.page; // coldest page.
		int cold_page = coldest_page_info.page;
		if (cold_page != cp) {
                    // Swap first page and coldest page.
                    // Slab_reassign reassigns first page of coldest 
                    // slab class to hottest slab class.
                    // Hence swapping first page and coldest page to remove
                    // coldest page during slab_reassign.
			void *ptr = s_cls->slab_list[cp];
			s_cls->slab_list[cp] = s_cls->slab_list[cold_page];
			s_cls->slab_list[cold_page] = ptr;

			unsigned int cp_nhits = s_cls->slab_stats[cp].nhits;
			rel_time_t cp_time = s_cls->slab_stats[cp].access_time;

			s_cls->slab_stats[cp].access_time = s_cls->slab_stats[cold_page].access_time;
			s_cls->slab_stats[cp].nhits = s_cls->slab_stats[cold_page].nhits;
			s_cls->slab_stats[cold_page].access_time = cp_time;
			s_cls->slab_stats[cold_page].nhits = cp_nhits;
		}
	}

	if (cp >= s_cls->slabs || cp < 0)
		cp = 0;
	slab_rebal.slab_start = s_cls->slab_list[cp];
	slab_rebal.slab_end = (char *) slab_rebal.slab_start
			+ (s_cls->size * s_cls->perslab);
	slab_rebal.slab_pos = slab_rebal.slab_start;
	slab_rebal.coldest_page = cp;
	slab_rebal.done = 0;
...
// time (without print statements)
//     - 1 page - 10,922 items - 175-225 microseconds
//     - 10 pages - 109,220 items - 1,000-1,700 microseconds (1-1.7 milliseconds)
//     - 100 pages - 1,092,200 items - 11,000-14,000 microseconds (11 - 14 milliseconds)
// time (print statements)
//     - 1 page - 10,922 items - 140-300 microseconds
//     - 10 pages - 109,220 items - 1,100 - 1,500 microseconds (1.1 - 1.5 milliseconds)
//     - 100 pages - 1,092,220 items - 12,000 microseconds
// time (print statements written to file)
//    - 1 page - 10,922 items - 400-600 microseconds
//    - 10 pages - 109,220 items - 1,500-3,000 microseconds (1.5-3 milliseconds)
//    - 100 pages - 1,092,220 items - 8,000 - 13,000 mircoseconds (8-13 milliseconds)

// Billy: method to traverse pages and keep track of hotness - percentage is used if you want to find the coldest x% of pages
// might want to take look at locks
void page_hotness(int percentage) {

	slabclass_t *curr_slab_class;

	// used to keep track of the sum of average times of all pages for a given slab class
	unsigned long long all_page_time = 0;
	uint32_t hv;

	// to keep track of the slab_class stats (to decide the hottest slab) 
	struct thread_stats thread_stats;
	...
	hottest_slab.slab = 0;
	hottest_slab.time = 0;
	hottest_slab.miss_rate = 0;

	coldest_page_info.slab = 0;
	coldest_page_info.page = 0;
	coldest_page_info.time = 0;
	coldest_page_info.nhits = 0;

	// Used if you want to write to file
	// FILE *f = fopen("path to file here", "w");
	// if (f == NULL) {
	//     printf("Error opening file!\n");
	//     exit(1);
	// }

	// If you want to keep track of x% coldest pages
	// int total_pages = get_all_pages();
	// int cold_amount = (total_pages / percentage);
	// if (cold_amount == 0 && total_pages < 10) {
	//     cold_amount = 1;
	// }
	// int cold_count = 0;
	// struct sp_tracker cold[cold_amount];
	// for(int z = 0; z < cold_amount; z++) {
	//     cold[z].slab = 0;
	//     cold[z].page = 0;
	//     cold[z].time = 0;
	// }

	for (int slab_class_id = 0; slab_class_id < MAX_NUMBER_OF_SLAB_CLASSES;
			slab_class_id++) {

		pthread_mutex_lock(&slabs_lock);
		curr_slab_class = &slabclass[slab_class_id];

		// check the amount of pages this slab class contains, if none, skip to the next slab class
		if (curr_slab_class->slabs == 0) {
			pthread_mutex_unlock(&slabs_lock);
			continue;
		}

		// find the start and end of the current page of the current slab class, and set the position to the start of the page
		page_start = curr_slab_class->slab_list[0];
		page_end = (char *) curr_slab_class->slab_list[0]
				+ (curr_slab_class->size * curr_slab_class->perslab);
		page_pos = page_start;

		int it_in_slbclspg = ((curr_slab_class->perslab)
				* (curr_slab_class->slabs)) - (curr_slab_class->sl_curr);
		int curr_page = 0;
		int item_count = 0;
		unsigned long long page_la_time = 0;
		int page_hits = 0;
		void *hold_lock = NULL;

		if (settings.verbose > 2) {
			printf(
					"slab:%d    total items:%d    free items:%d    valid items:%d\n",
					slab_class_id,
					curr_slab_class->perslab * curr_slab_class->slabs,
					curr_slab_class->sl_curr, it_in_slbclspg);
		}

		for (int curr_it_num = 0; curr_it_num < it_in_slbclspg; curr_it_num++) {

			/* Calcification: TODO: Lock item using hash lock */
			item *it = page_pos;
			assert((char *)it < (char *)page_end);
			item_chunk *ch = NULL;
			if (it->it_flags & ITEM_CHUNK) {
				
				page_pos = (char *) page_pos + curr_slab_class->size;
                                it_in_slbclspg++;
				...
				ch = (item_chunk *) it;
				// use the head chunk to find the item
				it = ch->head;
				assert(it->it_flags & ITEM_CHUNKED);
				/*if (ch != NULL && ch->head != NULL)
				{
					it = ch->head;
					assert(it->it_flags & ITEM_CHUNKED);
				}
			...
			// if we are looking at a blank, deleted, or expired item, skip it
			if (it->it_flags == 4 || it->time == 0) {
				page_pos = (char *) page_pos + curr_slab_class->size;
				it_in_slbclspg++;
				item_trylock_unlock(hold_lock);
				continue;
			...
			// can add or remove process started time to to get times
			page_la_time += (unsigned long long) it->time;
			/* Calcification */
			page_hits += it->nhits;
			// reset item nhits.
			it->nhits = 0;
			...
			// we have gone through all items for current_page
			if (page_pos >= page_end || curr_it_num + 1 == it_in_slbclspg) {

				unsigned long long avg_page_time = (page_la_time / item_count);
				/* Calcification */
				slabclass[slab_class_id].slab_stats[curr_page].access_time = avg_page_time;
				slabclass[slab_class_id].slab_stats[curr_page].nhits = page_hits;
				// this code is used when trying to find the coldest x% of pages
				// if the times aren't initially filled, fill them with the first pages
				// otherwise, see if the current page's time should replace any of the times currently stored
				// if(cold_count != cold_amount) {
				//     cold[cold_count].slab = slab_class_id;
				//     cold[cold_count].page = curr_page;
				//     cold[cold_count].time = avg_page_time;
				//     cold_count++;
				// }
				// else {
				//     unsigned long long hottest_time = cold[0].time;
				//     int to_replace = 0;
				//     for(int a = 0; a < cold_amount; a++) {
				//         if(cold[a].time > avg_page_time) {
				//             hottest_time = cold[a].time;
				//             to_replace = a;
				//         }
				//     }
				//     if(avg_page_time < hottest_time) {
				//         cold[to_replace].slab = slab_class_id;
				//         cold[to_replace].page = curr_page;
				//         cold[to_replace].time = avg_page_time;
				//     }
				// }

				/* Calcification */
				// finding the coldest page
				/*if(coldest_page_info.nhits == 0) {
				 	coldest_page_info.slab = slab_class_id;
				 	coldest_page_info.page = curr_page;
				 	coldest_page_info.time = avg_page_time;
				 	coldest_page_info.nhits = page_hits;
				 }
				 else if(page_hits < coldest_page_info.nhits) { // we can consider any factor, for now considering page hits
				 	coldest_page_info.slab = slab_class_id;
				 	coldest_page_info.page = curr_page;
				 	coldest_page_info.time = avg_page_time;
				 	coldest_page_info.nhits = page_hits;
				...
				*/
				 if(coldest_page_info.time == 0) {
					 ...
					 if(avg_page_time < coldest_page_info.time) { // we can consider any factor, for now considering page hits
						 coldest_page_info.slab = slab_class_id;
						 coldest_page_info.page = curr_page;
						 coldest_page_info.time = avg_page_time;
						 coldest_page_info.nhits = page_hits;
				...
				if (settings.verbose > 1) {
					printf("slab:%d    page:%d    items:%d    time:%llu\n",
							slab_class_id, curr_page, item_count,
							avg_page_time);
				}

				// if we aren't on the last page of this slab class, move to the next one, otherwise move to the next slab class
				if (curr_page + 1 != curr_slab_class->slabs) {
					curr_page++;
					page_start = curr_slab_class->slab_list[curr_page];
					page_end =
							(char *) curr_slab_class->slab_list[curr_page]
									+ (curr_slab_class->size
											* curr_slab_class->perslab);
					page_pos = page_start;
					all_page_time += avg_page_time;
					page_la_time = 0;
					page_hits = 0;
					item_count = 0;
				} else {
		...
		threadlocal_stats_aggregate(&thread_stats);
		pthread_mutex_unlock(&slabs_lock);
		// keeps track of the hottest slab
		// unsigned long long avg_slab_time = (all_page_time / curr_slab_class->slabs);
		
		 if(hottest_slab.miss_rate == 0) {
			 hottest_slab.slab = slab_class_id;
			 hottest_slab.miss_rate = (unsigned int)(((unsigned long long )thread_stats.slab_stats[slab_class_id].set_cmds - curr_slab_class->prev_misses)/ curr_slab_class->slabs);
		 }
		 else {
			 unsigned int miss_rate = (unsigned int)(((unsigned long long )thread_stats.slab_stats[slab_class_id].set_cmds - curr_slab_class->prev_misses)/ curr_slab_class->slabs);
			 if(miss_rate > hottest_slab.miss_rate) {
				 hottest_slab.slab = slab_class_id;
			 	 hottest_slab.miss_rate = miss_rate;
			 }
		 }

		 curr_slab_class->prev_misses = thread_stats.slab_stats[slab_class_id].set_cmds;
	...
	/*
	 if(settings.verbose > 1) {
	 printf("-----RESULTS-----\nHottest Slab: Slab %d\nColdest Page: Page %d of Slab %d\n", hottest_slab.slab, coldest_page.page, coldest_page.slab);
	 } */

	// used for printing out info for x% coldest pages
	// for(int e = 0; e < cold_amount; e++) {
	//     printf("Slab: %d    Page: %d    Time: %llu\n", cold[e].slab, cold[e].page, cold[e].time);
	// }
	pthread_mutex_unlock(&slabs_lock);
	// close if you write to file
	// fclose(f);
	return;
}
		...
		case MOVE_FROM_LRU:
			...
			if ((it->exptime != 0 && it->exptime < current_time)
				...
				save_item = 0;
					...
					&& (new_it = slab_rebalance_alloc(ntotal,
							slab_rebal.s_clsid)) == NULL) {
				/* Not a chunk of an item, and nomem. */
				save_item = 0;
				slab_rebal.evictions_nomem++;
			} else if (ch != NULL
					&& (new_it = slab_rebalance_alloc(s_cls->size,
							slab_rebal.s_clsid)) == NULL) {
...
static void slab_rebalance_finish(void) {
	if(settings.verbose >= 1) {
		printf("In slab_rebalance_finish\n");
	}
	slabclass_t *s_cls;
	slabclass_t *d_cls;
	int x;
	...
	s_cls->slabs--;
	// int cp = 0;
	int cp = slab_rebal.coldest_page; // coldest page.
	if (cp >= s_cls->slabs || cp <0)
		cp = 0;
	for (x = cp; x < s_cls->slabs; x++) {
		s_cls->slab_list[x] = s_cls->slab_list[x + 1];
		/* Calcification */ //coldest_page_info
		s_cls->slab_stats[x].access_time = s_cls->slab_stats[x+1].access_time;
		s_cls->slab_stats[x].nhits = s_cls->slab_stats[x+1].nhits;
	}

d_cls->slab_list[d_cls->slabs++] = slab_rebal.slab_start;
/* Calcification */
d_cls->slab_stats[d_cls->slabs].access_time = 0;
d_cls->slab_stats[d_cls->slabs].nhits = 0;
...
if (src < POWER_SMALLEST || src > power_largest || dst < SLAB_GLOBAL_PAGE_POOL
		|| dst > power_largest)
...
if ((ret = pthread_create(&rebalance_tid, NULL, slab_rebalance_thread, NULL))
		!= 0) {
