...
/* Calcification */
void slab_automove_custom_run(void *arg, int *src, int *dst){
    page_hotness(20);
    if (0 && (hottest_slab.miss_rate - coldest_page_info.miss_rate < 10)) {
    	*src = -1;
    	*dst = -1;
    	return;
    }
    *src = coldest_page_info.slab;
    *dst = hottest_slab.slab;
