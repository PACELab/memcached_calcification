...
static void *lru_maintainer_thread(void *arg) {
    ...
    /*Calcification */ 
    // rel_time_t last_hotness_check = 0; // to track when page_hotness() was called
    // rel_time_t hotness_wait = 15; // wait time between two successive page_hotness() calls 
	...
	/* Calcification */
        /* Enables the page hotness with the lru maintainer thread - can change how often it runs using the if statement and wait variable */
        /* 
	if((current_time - last_hotness_check) >= hotness_wait) {
            if(settings.verbose > 1) {                
                fprintf(stderr, "Checking page hotness...\n");
            }
            page_hotness(10);
            if(settings.verbose > 1) {
                fprintf(stderr, "Hotness check is complete!\n");
            }
            last_hotness_check = current_time;page_hotn
        } */
        if ((settings.slab_automove == 1 || settings.slab_automove == 3)
            ...
            else if (settings.slab_automove == 3)
            	slab_automove_custom_run(am, &src, &dst);

            if (settings.slab_automove >= 1 && settings.verbose >= 1) {
            	printf("Calcification: In Maintainer Thread - Automove: %d, "
            			"src-slab: %u, coldest-page:%u,  dst-slab: %u\n",
            			settings.slab_automove,  coldest_page_info.slab,
						coldest_page_info.page, hottest_slab.slab);
            }
